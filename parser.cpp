...
enum class GateType {
    AND, OR, NAND, NOR, XOR, XNOR, NOT, BUF, UNKNOWN
};

static inline string gateTypeToStr(GateType t) {
    switch (t) {
...
struct Gate {
    ...
    string instName;          // e.g. g1
    int outNet = -1;          // output net id
    vector<int> inNets;       // input net ids
};

struct Net {
    int id = -1;
    string name;
    int driverGate = -1;      // gate id driving this net (-1 if PI/const/un-driven)
    vector<int> sinkGates;    // gate ids consuming this net
    bool isPI = false;
    bool isPO = false;
    bool isConst = false;
    int constVal = -1;        // 0 or 1 if isConst
};
...
struct Circuit {
    ...
    int getNetId(const string& name) {
...
struct Bus {
    int id = -1;
    vector<int> members;      // net ids
};

struct BusInfo {
    vector<Bus> buses;
    vector<int> netToBus;     // size = circuit.nets.size(); -1 if not in any bus
};

struct ProblemInput {
    string c1File;
    vector<vector<string>> c1Buses; // list of bus member names

    string c2File;
    vector<vector<string>> c2Buses;
};
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        cerr << "Usage: " << argv[0] << " <problem_input_file>\n";
        return 1;
    }
    ...
    try {
        ...
        printCircuitSummary(c1);
        cout << "  C1 buses: " << b1.buses.size() << "\n";

        printCircuitSummary(c2);
        cout << "  C2 buses: " << b2.buses.size() << "\n";

        // Example: print each bus members by name
        for (int i = 0; i < (int)b1.buses.size(); i++) {
            cout << "C1 bus[" << i << "]: ";
            for (int nid : b1.buses[i].members) cout << c1.nets[nid].name << " ";
            cout << "\n";
        }
        for (int i = 0; i < (int)b2.buses.size(); i++) {
            cout << "C2 bus[" << i << "]: ";
            for (int nid : b2.buses[i].members) cout << c2.nets[nid].name << " ";
            cout << "\n";
        }
